...
Short answer: **Yes, and the loop is open at exactly one place.** The outcome side is already measured (harness outcome model, webhook-verified RECOVERED actions, holdout organic rates). The decision side is still three hand-set prior tables. Nothing in the repo today aggregates realized action outcomes per (action_type × failure_class) or feeds them back into `RevenueConfig`. The smallest defensible close is an environment-scoped measured-rate service with a MIN_CELL prior fallback, organic-baseline subtraction, and honesty labels — reusing patterns the repo already ships.
...
3. **Aggregated measured recovery (sums only).** `RevenueService.recovered_revenue` sums RECOVERED actions by window and by action_type (engine.py:319-365); `_actual_recovered` per incident (engine.py:512-521). UNKNOWN is counted separately, never silently included (engine.py:341-346). **No per-(action, class) conversion-rate aggregation exists anywhere** — grep for `conversion_rate|organic` across `backend/app/services/recovery` matches only a comment in strategies.py; nothing rates actions against their failure class.
...
|---|---|---|---|
| C1 | Measured action-outcome aggregation service (per env, action_type × failure_class, Wilson CI, MIN_CELL fallback) | HIGH-VALUE | **BUILD NOW** |
...
**BUILD NOW (one slice, three parts):** C1 measured-outcome aggregation + C4 organic subtraction + C3 environment-scoped override of the recoverability×effectiveness product with prior fallback and honesty labels, plus C2's persistence fix. This is the smallest change that turns intervention → outcome → evidence → future-decision into a genuinely *measured* loop: the evidence comes from the system's own verified action outcomes, the feedback is incremental-not-attributed, small-n and real_test starvation fall back to the documented priors automatically, and no ML, no exploration, and no circular import of simulator behavior is involved. **BUILD LATER:** C5 drift monitoring, C6 in-harness closed-loop validation (the Phase-B capstone demo). **REJECT:** C7 (prior laundering), C8 (decorative ML at this n), C9 (unsafe exploration), C10 (no data).
